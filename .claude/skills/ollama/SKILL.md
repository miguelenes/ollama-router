...
Always ask for the Ollama URL first — never assume `http://localhost:11434`.

Key facts:
- Default URL: `http://localhost:11434`
- Verify: `curl <OLLAMA_URL>/api/version`
- Validate: `python scripts/validate_connection.py <OLLAMA_URL>` (run from the skill directory)

Full installation walkthrough (per-platform installs, validation script
output, failure troubleshooting): `references/installation_and_setup.md`.
