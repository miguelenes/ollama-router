...
function App() {
  ...
  async function haltCloud() {
    if (!window.confirm('Halt Verda and RunPod? This terminates owned cloud GPUs. fleet.yaml hosts stay up.')) return
    setBusy(true); try { await api('/router/v1/cloud/halt', { method: 'POST', body: '{}' }); await refresh() } catch (e) { setError(e instanceof Error ? e.message : 'Halt failed') } finally { setBusy(false) }
  }
  ...
  return <main className="shell"><header><div><p className="eyebrow">CONTROL PLANE / {new Date().toLocaleTimeString()}</p><h1>Fleet readiness</h1></div><div className="actions"><button className="secondary" onClick={() => { setToken(''); setDraft('') }}>Lock</button><button className="danger" onClick={() => void haltCloud()} disabled={busy}>Halt cloud</button><button onClick={() => void recheck()} disabled={busy}>Recheck fleet</button></div></header>
