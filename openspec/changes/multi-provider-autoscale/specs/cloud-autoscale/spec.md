...
The system SHALL support a per-provider minimum billed lifetime and billing granularity. An idle instance MUST NOT be destroyed before its minimum billed lifetime has elapsed, and when a provider bills in coarse periods the teardown decision SHALL be evaluated so already-paid time is not discarded early. For a per-second-billed provider the minimum lifetime MAY be zero and teardown happens as soon as idle + grace allow.
