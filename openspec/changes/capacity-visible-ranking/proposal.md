...
- Specify **model placement**: default pull/ensure targets every node that fits the model’s **generate** class (not `RequestClass::Pull`). LARGE/MEDIUM skip unknown VRAM the same way ranking does.
...
- Ranking, placement, and class gates MUST distinguish **unknown** capacity from a measured CPU (`vram_gb = 0`, `gpus = 0`). Unmeasured VRAM MUST NOT be encoded as `0` for those gates or GPU-vs-CPU preference. Unknown MAY share the lowest-tier inflight cap number.
...
- `model-placement`: Default pull/ensure places the model on every generate-class-eligible node so load-share has a pool.
...
- `crates/ollama-router-core/src/routing/rank.rs` — Option VRAM/GPU, SMALL bands, MEDIUM/LARGE gates (WLC sort key stays).
...
- Tests: unknown vs GPU, LARGE unknown 503, classify `:latest`, placement skips unknown LARGE, existing WLC tests kept.
