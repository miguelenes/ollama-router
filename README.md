...
Release artifacts (`task agent:release` on Linux uses Docker `rust:1.97.1-slim-bookworm`; Darwin/Windows packages are the host recipes `task agent:release:macos` / `task agent:release:windows` — there is no GitHub Actions counterpart):
